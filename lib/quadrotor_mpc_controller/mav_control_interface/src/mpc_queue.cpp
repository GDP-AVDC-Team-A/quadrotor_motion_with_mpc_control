...
/*     AEROSTACK       
- Added remaining_path publisher (it sends remaining points from the original trajectory)
- Added assumed_pose publisher (it sends current target from the original trajectory)
*/
      ...
      queue_start_time_(0.0)
{
  ...
  remaining_path_publisher = nh_.advertise<nav_msgs::Path>( "motion_reference/remaining_path", 1 );
  assumed_pose_publisher = nh_.advertise<geometry_msgs::PoseStamped>( "motion_reference/assumed_pose", 1 );
...
/*     AEROSTACK       
- Added setRemainingPath function (it creates the remaining path to be sended)
*/
void MPCQueue::setRemainingPath(const mav_msgs::EigenTrajectoryPoint& point)
{
  remaining_path.poses.clear();
  geometry_msgs::PoseStamped pose_path;
  pose_path.pose.position.x = point.position_W.x();
  pose_path.pose.position.y = point.position_W.y();
  pose_path.pose.position.z = point.position_W.z();
  pose_path.pose.orientation.x = point.orientation_W_B.x();
  pose_path.pose.orientation.y = point.orientation_W_B.y();
  pose_path.pose.orientation.z = point.orientation_W_B.z();
  pose_path.pose.orientation.w = point.orientation_W_B.w(); 
  remaining_path.header.stamp = ros::Time::now();
  remaining_path.poses.push_back(pose_path);
...
- Added setRemainingPath function (it creates the remaining path to be sended)
*/
void MPCQueue::setRemainingPath(const mav_msgs::EigenTrajectoryPointDeque& trajectory)
{
  //Transform into nav_msgs path for topic remaining path
  remaining_path.header.stamp = ros::Time::now();
  remaining_path.poses.clear();

  for (int i = 0; i < trajectory.size(); i++) {
    geometry_msgs::PoseStamped pose_path;
    pose_path.pose.position.x = trajectory.at(i).position_W.x();
    pose_path.pose.position.y = trajectory.at(i).position_W.y();
    pose_path.pose.position.z = trajectory.at(i).position_W.z();
    pose_path.pose.orientation.x = trajectory.at(i).orientation_W_B.x();
    pose_path.pose.orientation.y = trajectory.at(i).orientation_W_B.y();
    pose_path.pose.orientation.z = trajectory.at(i).orientation_W_B.z();
    pose_path.pose.orientation.w = trajectory.at(i).orientation_W_B.w();
    remaining_path.poses.push_back(pose_path);
  }
  publishPathReferences();
}
...
/*     AEROSTACK       
- Added checkPathReferences function (it removes points from the remaining path)
*/
void MPCQueue::checkPathReferences(Eigen::Vector3d position_ref, double yaw_ref){
  geometry_msgs::PoseStamped first_point = remaining_path.poses.front();
  Eigen::Quaterniond yaw_quaterniond(first_point.pose.orientation.w, first_point.pose.orientation.x, 
                                      first_point.pose.orientation.y, first_point.pose.orientation.z);
  double yaw_diff = std::abs(mav_msgs::yawFromQuaternion(yaw_quaterniond) - yaw_ref);
  double pose_dist_dff = std::sqrt(std::pow(first_point.pose.position.x-position_ref.x(),2)
